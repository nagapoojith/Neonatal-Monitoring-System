...
const HospitalMap: React.FC = () => {
  ...
  useEffect(() => {
    const fetchApiKey = async () => {
      try {
        const { data, error } =
          await supabase.functions.invoke("get-tomtom-key");
        if (error) {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (error) => {
        ...
        if (error.code === error.PERMISSION_DENIED) {
          toast.error(
            "Location permission denied. Please enable location access to get directions.",
          );
  ...
  const getCurrentLocation = useCallback(() => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (error) => {
        console.error("Geolocation error:", error);
        setLocationError(
          "Unable to get your location. Please enable location access or search manually.",
        );
        setIsLoading(false);
        setHospitals(fallbackHospitals);
        setSelectedHospital(fallbackHospitals[0]);
  ...
  const renderStars = (rating: number) => {
    ...
    for (let i = 0; i < fullStars; i++) {
      stars.push(
        <Star
          key={i}
          className="w-3.5 h-3.5 fill-status-warning text-status-warning"
        />,
      );
    }
    if (hasHalfStar) {
      stars.push(
        <Star
          key="half"
          className="w-3.5 h-3.5 fill-status-warning/50 text-status-warning"
        />,
      );
    }
  ...
  return (
    <div className="space-y-4">
      <div className="portal-glow-soft rounded-xl border border-status-critical/30 bg-status-critical-bg p-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-foreground">
              Call <strong className="font-bold">108</strong> (Ambulance) or{" "}
              <strong className="font-bold">112</strong> (Emergency)
              immediately.
            </p>
          </div>
        </div>
      ...
      <Card className="portal-glow border-primary/15 bg-gradient-to-br from-background to-accent/20 shadow-lg">
        <CardHeader className="pb-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <CardDescription>
                Locate pediatric and children's hospitals near you
              </CardDescription>
            </div>
          </div>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex gap-2">
            <Button
              ...
              disabled={isLoading}
              className="portal-glow-soft gap-2"
            >
              {isLoading ? (
          ...
          <div className="flex gap-2">
            <Input
              ...
              className="portal-glow-soft flex-1 border-primary/15"
          ...
          {locationError && (
            <p className="portal-glow-soft rounded-lg bg-status-warning-bg p-3 text-sm text-status-warning">
              {locationError}
            </p>
          )}
        </CardContent>
      ...
      <Card className="portal-glow overflow-hidden border-primary/15 shadow-lg">
            ...
            <div>
              ...
              <CardDescription>
                {selectedHospital
                  ...
                  : "Select a hospital to view on map"}
              </CardDescription>
            </div>
            {showingRoute && (
              <Button
                ...
                size="sm"
                onClick={clearRoute}
                className="portal-glow-soft gap-1"
              >
                <X className="w-4 h-4" />
                Clear Route
              </Button>
        ...
        <CardContent className="p-0">
          <div className="h-[350px] w-full relative">
            ...
            {routeInfo && (
              <div className="portal-glow-soft absolute left-4 right-4 top-4 rounded-lg border border-primary/20 bg-background/95 p-3 shadow-lg backdrop-blur-sm">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    <p className="font-semibold text-sm text-foreground">
                      Route to {routeInfo.hospitalName}
                    </p>
            ...
            {selectedHospital && !showingRoute && (
              <div className="portal-glow-soft absolute bottom-4 left-4 right-4 rounded-lg border bg-background/95 p-3 shadow-lg backdrop-blur-sm">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <Button
                    ...
                    className="portal-glow-soft flex-shrink-0"
                  >
                    {isRouteLoading ? (
      ...
      <Card className="portal-glow border-primary/15 shadow-lg">
        <CardHeader className="pb-3 border-b bg-gradient-to-r from-accent/30 to-primary/5">
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="text-lg flex items-center gap-2">
          ...
          <ScrollArea className="h-[350px]">
            <div className="divide-y divide-border/50">
              {hospitals.map((hospital) => (
                <div
                  key={hospital.id}
                  className={`portal-glow-soft cursor-pointer p-4 ${
                ...
                >
                  ...
                  <div className="flex gap-2 flex-wrap">
                    <Button
                      ...
                      disabled={isRouteLoading}
                      className="portal-glow-soft gap-1.5 flex-1 min-w-[120px]"
                    >
                      {isRouteLoading ? (
                        <Loader2 className="w-4 h-4 animate-spin" />
                      ) : (
                    ...
                    {hospital.phone && (
                      <Button
                        ...
                        className="portal-glow-soft gap-1.5 border-status-normal/30 text-status-normal hover:bg-status-normal-bg flex-1 min-w-[100px]"
                      >
