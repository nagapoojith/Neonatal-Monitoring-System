...
const CRY_MESSAGES: Record<CryClassification, string> = {
  normal:
    "Cry pattern appears typical for a newborn. No specific concerns identified.",
  discomfort:
    "Cry pattern suggests possible discomfort. Caregiver attention recommended.",
  pain: "Cry pattern indicates potential distress. Immediate caregiver attention recommended.",
};

const CRY_COLORS: Record<
  CryClassification,
...
const CryDetectionPage = () => {
  ...
  const [history, setHistory] = useState<CryEvent[]>(() => {
    ...
    babies.slice(0, 2).forEach((baby) => {
      const classes: CryClassification[] = [
        "normal",
        "discomfort",
        "normal",
        "pain",
      ...
      classes.forEach((cls, i) => {
        events.push({
          ...
          timestamp: new Date(
            Date.now() - i * 1800000 - Math.random() * 3600000,
          ),
  ...
  const analyzeAudio = useCallback((): CryEvent => {
    const classifications: CryClassification[] = [
      "normal",
      "discomfort",
      "pain",
    ];
    ...
    try {
      ...
      const update = () => {
        ...
        setAudioLevel(
          Math.min(
            100,
            (dataArray.reduce((a, b) => a + b, 0) / dataArray.length / 128) *
              100,
          ),
        );
  ...
  // Stats
  const babyHistory = selectedBaby
    ? history.filter((h) => h.babyId === selectedBaby)
    : history;
  const normalCount = babyHistory.filter(
    (h) => h.classification === "normal",
  ).length;
  const discomfortCount = babyHistory.filter(
    (h) => h.classification === "discomfort",
  ).length;
  const painCount = babyHistory.filter(
    (h) => h.classification === "pain",
  ).length;
  const avgConfidence =
    babyHistory.length > 0
      ? Math.round(
          babyHistory.reduce((s, h) => s + h.confidence, 0) /
            babyHistory.length,
        )
      : 0;
      ...
      <div className="space-y-6 animate-fade-in">
        <div>
          ...
          <p className="text-muted-foreground">
            ML-based neonatal cry pattern analysis
          </p>
        </div>
            ...
            <Card className="card-medical overflow-hidden">
              ...
              <CardContent className="p-5 space-y-4">
                ...
                {currentResult && !isAnalyzing && (
                  <div
                    className={cn(
                      ...
                      CRY_COLORS[currentResult.classification].bg,
                    )}
                  >
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-3">
                        <span className="text-3xl">
                          {CRY_COLORS[currentResult.classification].icon}
                        </span>
                        <div>
                          <span className="font-bold text-lg text-foreground capitalize">
                            {currentResult.classification} Cry
                          </span>
                          <p className="text-xs text-muted-foreground">
                            Duration: ~{currentResult.duration}s
                          </p>
                        </div>
                      </div>
                      {isDoctorOrSenior && (
                        <Badge
                          className={cn(
                            ...
                            CRY_COLORS[currentResult.classification].badge,
                          )}
                        >
                    ...
                    <p className="text-xs text-muted-foreground mt-2">
                      Detected at {currentResult.timestamp.toLocaleTimeString()}
                    </p>
                  </div>
                )}
                ...
                <div className="flex items-start gap-2 p-3 rounded-lg bg-muted/50 border border-border/30">
                  ...
                  <p className="text-xs text-muted-foreground">
                    This is an assistive tool only. Always consult healthcare
              ...
              <CardHeader className="pb-3 border-b border-border/50">
                <div className="flex items-center justify-between">
                  <CardTitle className="text-base font-bold flex items-center gap-2">
                    <Clock className="w-4 h-4 text-primary" /> Recent Cry
                    Activity
                  </CardTitle>
                ...
                {babyHistory.slice(0, 15).map((event) => (
                  <div
                    ...
                    className={cn(
                      ...
                      CRY_COLORS[event.classification].bg,
                    )}
                  >
                    <span className="text-xl">
                      {CRY_COLORS[event.classification].icon}
                    </span>
          ...
          <div className="space-y-4">
            <Card className="card-medical">
              <CardHeader className="pb-3">
                <CardTitle className="text-base font-bold flex items-center gap-2">
                  <BarChart3 className="w-4 h-4 text-primary" /> Detection
                  Summary
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="space-y-3">
                  <div className="flex items-center justify-between p-3 rounded-xl bg-status-normal-bg">
                    ...
                    <span className="text-lg font-bold text-status-normal">
                      {normalCount}
                    </span>
                  </div>
                      ...
                      .map((event, i) => (
                        <div
                          ...
                          className={cn(
                            ...
                            CRY_COLORS[event.classification].badge,
                          )}
                          style={{
                            height: `${event.confidence}%`,
                            opacity: isDoctorOrSenior ? 1 : 0.5,
                          }}
                        />
                      ))}
                  </div>
              ...
              <CardContent className="p-4">
                <div className="flex items-start gap-2 p-3 rounded-lg bg-status-warning-bg border border-status-warning/20">
                  ...
                  <p className="text-xs text-foreground">
                    Cry detection is assistive technology only and does not
                    constitute a medical diagnosis. Always verify with clinical
                    assessment.
                  </p>
                </div>
              </CardContent>
